...
import random
import requests
from collections import deque # BFS용 큐 추가
...
def fetch_live_kickboards(center_lat, center_lng, radius_m, count):
    ...
    for i in range(count):
        ...
        kickboards.append({
            "id": f"EB-{random.randint(1000, 9999)}",
            "lat": lat,
            "lng": lng,
...
# ==========================================
# 3. [심화] 최적 경로 탐색 (BFS 기반 완전탐색 & Greedy 하이브리드)
# ==========================================
def calculate_optimal_route(depot, kickboards):
    """
    전기자전거 대수가 적을 때(10대 이하)는 BFS(너비 우선 탐색) 기반 완전탐색으로
    100% 최적 경로를 찾고, 연산량이 폭발할 위험이 있을 때는
    탐욕 알고리즘(Greedy)으로 자동 전환합니다.
    """

    # 1. 안전장치: 자전거가 11대 이상이면 탐욕 알고리즘 작동 (서버 다운 방지)
    # 웹 환경이므로 20대 제한은 메모리 과부하를 일으킬 수 있어 10대 제한으로 유지합니다.
    ...
    # 2. BFS 기반 완전탐색 (Branch and Bound)
    best_path = []
    min_distance = float('inf')

    # 큐에 저장할 상태: (현재 위치, 미방문 목록, 지금까지의 경로, 지금까지의 거리)
    queue = deque()
    queue.append((depot, kickboards, [], 0))

    while queue:
        current_node, unvisited, current_path, current_dist = queue.popleft()

        # [핵심] 가지치기 (Pruning):
        # 지금까지 찾은 최단거리보다 이미 길어졌다면 더 볼 필요 없으니 버림
        if current_dist >= min_distance:
            continue

        # 모든 자전거를 다 방문했을 때 → 완성된 경로 후보
        if not unvisited:
            if current_dist < min_distance:
        ...
        # 다음 단계 후보들을 한 단계씩 모두 큐에 넣음 (너비 우선 확장)
        for i, next_node in enumerate(unvisited):
            ...
            queue.append((next_node, next_unvisited, current_path + [next_node], current_dist + dist))
...
kickboard_count = st.sidebar.slider("해당 구역 내 예상 방치 대수", min_value=5, max_value=25, value=9)
refresh_btn = st.sidebar.button("🔄 실시간 API 및 도로망 재탐색", use_container_width=True)

st.title("🌐 제주 스마트시티: 전기자전거 BFS 라우팅 시스템")
...
st.info("알고리즘: 10대 이하일 때는 **BFS 완전탐색**으로 100% 최적 경로를 계산하며, 11대 이상부터는 서버 보호를 위해 **Greedy 알고리즘**으로 전환됩니다.")
